const modifier = (text) => {
    ...
    if (info.actionCount > 1 && state.inputBot) {
        ...
        let chkAtt = info?.inputEvaluation[statConfig.botOutputs.stat]
        ...
        if (state.showDC) { // if the display is on (iE, state.showDC == true)...
            state.message = statConfig.statList[chkAtt].icon + " DC " + chkDC + ": " + chkCuz // ...show the attribute, DC and reason in state.message
